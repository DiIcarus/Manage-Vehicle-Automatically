...
def plate_processing(LpImg):
	if (len(LpImg)): #check if there is at least one license image
		...
		#done 2
		cv2.imshow("abc",test_roi)
		cv2.waitKey(1000)
		for i in range(len(crop_characters)):
				cv2.imshow("aaa",crop_characters[i])
				cv2.waitKey(1000)
...
def demo1(index):
	...
	plate_processing(LpImg)

for i in range( len(IMAGE_PATHS)):
  demo1(i)

def demoMultiPlate():
	multiple_plates_image = "./Core/demo/multi.png"
